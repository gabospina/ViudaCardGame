...
class CardGameGUI:
    instances = []
    CARD_HEIGHT = 75  # 75  # Define the height of the cards
    CARD_WIDTH = 50  # 50
    SPACING = 250
    ...
    def __init__(self, root, player, other_player, reveal_deck=None):
        ...
        self.root.geometry("500x300")  # Adjust the window size
    ...
    def create_reveal_frame(self):
        ...
        for i, card in enumerate(self.reveal_deck):
            ...
            # lbl.bind("<B1-Motion>", lambda event, label=lbl: self.player_drag_motion(event, label))
    ...
    def reveal_drag_start(self, event, idx):
        ...
        self.reveal_dragged_card.tk_image = card.tk_image
        self.reveal_dragged_card = self.dragged_card
    ...
    def reveal_drag_end(self, event, label):
        ...
        #
        # # Remove the dropped card from the reveal deck & IF this line remove reveal cards DUPLICATE
        ...
        if hand_frame_bbox is not None and x >= hand_frame_bbox[0] and x <= hand_frame_bbox[2] and y >= hand_frame_bbox[
            1] and y <= hand_frame_bbox[3]:
            if self.reveal_dragged_card.label in self.reveal_card_labels:
                self.reveal_dragged_card.label.destroy()
                self.reveal_card_labels.remove(self.reveal_dragged_card.label)
    ...
    def player_drag_motion(self, event, label):
        ...
        #
        #     # Define left and right bounds for dragging within player's hand
        ...
        #         # If dragging more on the x-axis, trigger dragging within player's hand
        #         self.on_drag_motion(event, self.dragged_card.idx, left_bound, right_bound)
        #     else:
        #         # If dragging more on the y-axis, continue dragging card onto reveal cards
