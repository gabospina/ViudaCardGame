...
class CardGameGUI:
    instances = []
    dragging = False  # Define the dragging attribute
    ...
    def create_reveal_frame(self):
        ...
        # Bind pass_button_clicked method to the click event of the pass button in the second window
        self.pass_button = tk.Button(self.reveal_frame, text="Pass",
                                     command=lambda: self.pass_turn(next_window_index=1))

        self.pass_button.pack(side=tk.TOP, padx=(500, 0), pady=10)

        # self.pass_button.bind("<Button-1>", lambda event: self.pass_button_clicked(event))
        # self.pass_button.bind("<Button-1>", self.pass_button_clicked)
    ...
    def pass_button_clicked(self, event):
        # print("Pass button clicked on the second window")
        print("pass_button_clicked method called from window", str(CardGameGUI.instances.index(self)))
        # This method is called when the pass button in the first window is clicked
        # Call pass_turn method with next_window_index argument to activate buttons in the second window
        # self.pass_turn(next_window_index=1)
        self.pass_turn(next_window_index=(CardGameGUI.instances.index(self) + 1) % len(CardGameGUI.instances))

    def pass_turn(self, next_window_index):
        # print("pass_turn method called 1-4")
        # # Check if the current instance is the first window
        # if CardGameGUI.instances.index(self) == 0:
        #     print("Current instance is the first window 2-4")
        #     # Find the next instance (specified by the next_window_index)
        #     if next_window_index < len(CardGameGUI.instances):
        #         next_instance = CardGameGUI.instances[next_window_index]
        #         # Print out the state of the buttons before enabling
        #         print("State of buttons in next window before enabling 3-4:",
        #               next_instance.reveal_button.cget('state'),
        #               next_instance.pass_button.cget('state'))
        #         # Enable pass button in the specified window
        #         next_instance.pass_button.config(state=tk.NORMAL)
        #         # Change pass button color
        #         next_instance.pass_button.config(bg="red")
        #         # Print out the state of the buttons after enabling
        #         print("State of buttons in next window after enabling 4-4:",
        #               next_instance.reveal_button.cget('state'),
        #               next_instance.pass_button.cget('state'))
        #     elif next_window_index != 1:
        #         return
        # else:
        #     # Disable pass button in all windows except the specified window
        #     for instance in CardGameGUI.instances:
        #         if instance != self:
        #             instance.pass_button.config(state=tk.DISABLED)
        #             instance.reveal_button.config(state=tk.DISABLED)  # Disable reveal button
        #     # Enable pass button in the specified window
        #     self.pass_button.config(state=tk.NORMAL)
        #     self.pass_button.config(bg="red")
        def pass_turn(self, next_window_index):
            # Check if the current instance is the first window
            if CardGameGUI.instances.index(self) == 0:
                # Find the next instance (specified by the next_window_index)
                next_instance = CardGameGUI.instances[next_window_index]
                # Activate pass button in the specified window
                next_instance.pass_button.config(state=tk.NORMAL, bg="green")
            else:
                # Deactivate pass button in the current window
                self.pass_button.config(state=tk.DISABLED, bg="red")

        # Print contents of CardGameGUI.instances to verify list maintenance
        print("Instances list:", CardGameGUI.instances)
    ...
    def update_buttons_visibility(self):
        ...
        #     self.pass_button.config(state=tk.NORMAL)
        # else:
        #     # Deactivate buttons in other windows
        ...
        #     # Deactivate buttons in the third and fourth windows
        #     if len(self.__class__.instances) > 2:
        #         for i in range(2, len(self.__class__.instances)):
        #             instance = self.__class__.instances[i]
        #             instance.reveal_button.config(state=tk.DISABLED)
        #             instance.pass_button.config(state=tk.DISABLED)
        #         # Activate buttons in the second window
        #     if self == self.__class__.instances[1]:
        #         self.reveal_button.config(state=tk.NORMAL)
        #         self.pass_button.config(state=tk.NORMAL)
        if CardGameGUI.instances.index(self) == 0:
            # Enable buttons in the first window
            self.reveal_button.config(state=tk.NORMAL)
            self.pass_button.config(state=tk.NORMAL, bg="green")
        else:
            # Deactivate buttons in other windows
            self.reveal_button.config(state=tk.DISABLED)
            self.pass_button.config(state=tk.DISABLED, bg="red")
    ...
    def reveal_cards(self):
        ...
        # Disable the reveal button in all windows
        for instance in CardGameGUI.instances:
            instance.reveal_button.config(state=tk.DISABLED)
