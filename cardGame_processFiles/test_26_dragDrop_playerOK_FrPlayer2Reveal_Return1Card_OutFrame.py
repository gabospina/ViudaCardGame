...
class CardGameGUI:
    instances = []
    CARD_HEIGHT = 50  # 75  # Define the height of the cards
    CARD_WIDTH = 50  # 50
    SPACING = 450
    ...
    def __init__(self, root, player, other_player, reveal_deck=None):
        ...
        self.drop_targets = []
        self.offset = (0, 0)  # Initialize offset attribute
    ...
    def start_drag(self, event):
        ...
        self.offset = (event.x_root - self.dragging.winfo_rootx(),
                       event.y_root - self.dragging.winfo_rooty())
    ...
    def drag(self, event):
        ...
        if self.dragging:
            x, y = event.x_root - self.offset[0], event.y_root - self.offset[1]
            self.dragging.place(x=0, y=max(0, min(y, self.hand_frame.winfo_height() - self.dragging.winfo_height())))

    def drop(self, event):
        if self.dragging:
            x, y = event.x_root - self.offset[0], event.y_root - self.offset[1]
            hand_frame_bbox = self.hand_frame.bbox()
            card_height = self.dragging.winfo_height()
            if hand_frame_bbox is not None and hand_frame_bbox[0] <= x <= hand_frame_bbox[2] \
                    and hand_frame_bbox[1] <= y <= hand_frame_bbox[3] - card_height:
                print("Card dropped inside Player's Hand")
            else:
                print("Not enough space in Player's Hand for the dragged card")
            self.dragging.place_forget()
            self.dragging = None
    ...
    def update_hand_display(self):
        ...
        for i, card in enumerate(self.player.hand):
            ...
            # # Bind events to the label drag & drop inside the player's hand
    ...
    def create_player_hand_frame(self):
        ...
        # self.hand_frame = tk.Frame(self.root, bg="green", width=CardGameGUI.CARD_WIDTH * 6, height=CardGameGUI.CARD_HEIGHT, borderwidth=6, relief=tk.RIDGE)
        # self.hand_frame = tk.Frame(self.root, bg="green", width=300, height=450, borderwidth=6, relief=tk.RIDGE)

        frame_width = 6 * CardGameGUI.CARD_WIDTH  # Assuming each card has a width of 50 pixels
        frame_height = CardGameGUI.CARD_HEIGHT
        self.hand_frame = tk.Frame(self.root, bg="green", width=frame_width, height=frame_height, borderwidth=6,
                                   relief=tk.RIDGE)
