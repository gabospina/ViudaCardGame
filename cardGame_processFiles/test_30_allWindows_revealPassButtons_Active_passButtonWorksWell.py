...
class CardGameGUI:
    ...
    def __init__(self, root, player, other_player, reveal_deck=None):
        ...
        if len(self.__class__.instances) == 1:
            # Create buttons only in the first instance
            self.create_buttons()
    ...
    def create_reveal_frame(self):
        ...
        # Check the index of the current instance to determine whether to create buttons
        # instance_index = CardGameGUI.instances.index(self)
        # if instance_index == 0:
            # Create the reveal and pass buttons on this window
        self.reveal_button = tk.Button(self.reveal_frame, text="Reveal", command=self.reveal_cards)
        self.reveal_button.pack(side=tk.TOP, padx=(0, 500), pady=10)
        self.pass_button = tk.Button(self.reveal_frame, text="Pass", command=self.pass_turn)
        self.pass_button.pack(side=tk.TOP, padx=(500, 0), pady=10)
        # else:
            # If this is the first window, set reveal_button and pass_button to None
        # self.reveal_button = None
        # self.pass_button = None
    ...
    def create_buttons(self):
        if not hasattr(self, 'reveal_button') and not hasattr(self, 'pass_button'):
            # Create the reveal and pass buttons only if they haven't been created yet
            self.reveal_button = tk.Button(self.reveal_frame, text="Reveal", command=self.reveal_cards)
            self.pass_button = tk.Button(self.reveal_frame, text="Pass", command=self.pass_turn)
    ...
    def disable_buttons(self):
        # Disable buttons in all windows except the first window
        self.reveal_button.config(state=tk.DISABLED)
        self.pass_button.config(state=tk.DISABLED)

    def pass_turn(self):
        next_instance_idx = CardGameGUI.instances.index(self) + 1
        if next_instance_idx < len(CardGameGUI.instances):
            next_instance = CardGameGUI.instances[next_instance_idx]

            # Destroy the reveal and pass buttons on the current window
            if self.reveal_button:
                self.reveal_button.destroy()
            if self.pass_button:
                self.pass_button.destroy()

            # Create and show the reveal and pass buttons on the next window
            # next_instance.reveal_button = tk.Button(next_instance.reveal_frame, text="Reveal",
            #                                         command=next_instance.reveal_cards)
            # next_instance.reveal_button.pack(side=tk.TOP, padx=(0, 500), pady=10)
            #
            # next_instance.pass_button = tk.Button(next_instance.reveal_frame, text="Pass",
            #                                       command=next_instance.pass_turn)
            # next_instance.pass_button.pack(side=tk.TOP, padx=(500, 0), pady=10)

            # Update the buttons' visibility on the next window
            next_instance.update_buttons_visibility(next_instance)
        else:
            print("Last window reached, no more buttons to activate")

    def update_buttons_visibility(self, instance):
        if instance == self:
            if not self.reveal_button:
                self.reveal_button = tk.Button(self.reveal_frame, text="Reveal", command=self.reveal_cards)
                self.reveal_button.pack(side=tk.TOP, padx=(0, 500), pady=10)
            if not self.pass_button:
                self.pass_button = tk.Button(self.reveal_frame, text="Pass", command=self.pass_turn)
                self.pass_button.pack(side=tk.TOP, padx=(500, 0), pady=10)
        else:
            if self.reveal_button:
    ...
    def reveal_cards(self):
        ...
        #
        #     # Destroy the reveal button and pass button on the first window
        #     if self.reveal_button:
        #         self.reveal_button.destroy()
        #     if self.pass_button:
        #         self.pass_button.destroy()
        # Destroy the reveal and pass buttons in all instances
        for instance in self.__class__.instances:
            if instance.reveal_button:
                instance.reveal_button.destroy()
            if instance.pass_button:
