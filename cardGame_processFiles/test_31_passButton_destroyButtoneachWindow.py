...
class CardGameGUI:
    ...
    def pass_turn(self):
        ...
        # # Check if there are more windows to activate buttons
        # if len(self.__class__.instances) > 1:
        #     # Get the index of the next window
        #     next_index = self.__class__.instances.index(self) + 1

            # Activate buttons in the next window
            # if next_index < len(self.__class__.instances):
            #     next_instance = self.__class__.instances[next_index]
            #     next_instance.create_buttons()
            #     next_instance.update_buttons_visibility()
    ...
    def update_buttons_visibility(self):
        ...
        if len(self.__class__.instances) == 1:
            ...
            # Disable buttons in other windows
            self.reveal_button.config(state=tk.DISABLED)
            self.pass_button.config(state=tk.DISABLED)
