...
class CardGameGUI:
    ...
    def create_reveal_frame(self):
        ...
        self.pass_button = tk.Button(self.reveal_frame, text="Pass",
                                     command=lambda event=None: self.pass_button_clicked(event))
    ...
    def pass_button_clicked(self, event):
        ...
        # This method is called when the pass button in the current window is clicked
        # Call pass_turn method with next_window_index argument to activate buttons in the next window
        next_window_index = (CardGameGUI.instances.index(self) + 1) % len(CardGameGUI.instances)
        self.pass_turn(next_window_index)

    def pass_turn(self, next_window_index):
        print(f"pass_turn method called with next_window_index: {next_window_index}")
        current_index = CardGameGUI.instances.index(self)
        # Deactivate buttons in the current window
        self.pass_button.config(state=tk.DISABLED, bg="red")
        self.reveal_button.config(state=tk.DISABLED, bg="red")
        # Activate buttons in the next window
        next_instance = CardGameGUI.instances[next_window_index]
        next_instance.pass_button.config(state=tk.NORMAL, bg="green")
        next_instance.reveal_button.config(state=tk.NORMAL, bg="green")
    ...
    def update_buttons_visibility(self):
        print(f"update_buttons_visibility method called for window {CardGameGUI.instances.index(self)}")
        if CardGameGUI.instances.index(self) == 0:
            print("Enabling buttons in the first window")
            # Enable buttons in the first window
            self.reveal_button.config(state=tk.NORMAL, bg="green")
            self.pass_button.config(state=tk.NORMAL, bg="green")
        else:
            print(f"Deactivating buttons in window {CardGameGUI.instances.index(self)}")
