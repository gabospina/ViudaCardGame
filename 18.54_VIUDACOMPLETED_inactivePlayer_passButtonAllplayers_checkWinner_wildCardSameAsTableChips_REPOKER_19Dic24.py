...
from PyQt5.QtWidgets import (
    ...
    QDockWidget,
    QMainWindow,
    QPushButton, QInputDialog, QMessageBox, QDialog, QLineEdit, QListWidget, QListWidgetItem
    )
...
class PlayerWindow(QMainWindow):
    ...
    # All players clicked pass button =================

    def is_current_player_turn2ok(self):
        return self.main_window.current_player_number == self.player_number
    ...
    def get_next_player(self):
        ...
        if self.main_window and self.main_window.players:
            next_index = (self.main_window.player_index + 1) % len(self.main_window.players)
    ...
    def reveal_cards(self, table_chips):
        try:
            ...
            if next_player_window:
                ...
                # next_player_index = next_player_window.player_index
    ...
    def call_cards(self):
        ...
        if next_player_window:
            QTimer.singleShot(0, lambda: self.main_window.check_end_hand(current_player_number, self.main_window.table_chips))
    ...
    def end_current_player_turn(self):
        ...
        next_player_number = (self.player_number % len(self.main_window.player_windows)) + 1
        next_player_window = next(
            window for window in self.main_window.player_windows if window.player_number == next_player_number)
...
class Dealer(QObject):
    ...
    def evaluate_possible_handPOKER(self, values, suits, wildcards):
        count = Counter(values)
        most_common = count.most_common()
        # print(f"Dealer: eval_hand - Evaluating with values: {values} -> Most common: {most_common}")
        ...
        if is_straight and is_flush:
            ...
            return poker_result
        elif most_common and most_common[0][1] == 4:
...
class GameWindow(QMainWindow):
    def __init__(self, app, num_players, player_names, parent=None):
        ...
        # self.all_player_cards = [[] for _ in range(num_players)]  # Initialize with empty lists for each player
    ...
    def start_first_hand(self):
        ...
        for player_window in self.player_windows:
            ...
            player_window.set_initial_button_states(current_player_number=self.current_player_number,
                                                    call_button_clicked=False)  # Set initial button states
    ...
    def check_end_hand1ok(self, player_number):
        """Check if the hand should end based on the number of turns after the call button was clicked."""
        try:
            ...
            if hasattr(self, 'call_button_clicked') and self.call_button_clicked:
                ...
                if self.turns_after_call >= active_players:
                    ...
                    self.end_hand(self.table_chips)
                    return
    ...
    def handle_end_game(self):
        try:
            print("Checking for end of game...")
            if len(self.player_windows) == 1:
                final_winner = self.player_windows[0]
                print(f"Game over! Final winner is {final_winner.player_name} with {final_winner.player_chips} chips.")
                self.final_end_game()  # Declare the remaining player as the winner
            else:
                print("Game continues, more than one active player left.")

        except Exception as e:
            print(f"Error in handle_end_game: {e}")
    ...
    def set_current_player1(self, player_number):
    ...
    def end_current_player_turn(self):
        ...
        if current_player_window:
            ...
            next_player_number = (self.current_player_number % len(self.player_windows)) + 1
