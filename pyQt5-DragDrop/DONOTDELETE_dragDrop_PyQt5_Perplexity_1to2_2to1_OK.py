...
class MainWindow(QMainWindow):
    ...
    def handle_drag1_drop(self, item_data):
        print("handle_drag1_drop called")
        if len(item_data) == 5 and self.drag2.item_count < 6:
            ...
            print("Max items six in dock2")

    def handle_drag2_drop(self, item_data):
        print("handle_drag2_drop called")
        # Check if dock2 has exactly 6 items and dock1 has less than 5 items
        if len(item_data) == 6 and self.drag1.item_count < 5:
            item = DragItem(item_data[-1])
            item.set_data(item_data[-1])
            self.drag1.add_item(item)

            # Remove dropped item from dock2
            self.drag2.remove_item(self.drag2.blayout.itemAt(len(item_data) - 1).widget())
        elif len(item_data) == 5:
            # Print the message "Max items five in dock1"
            print("Max items five in dock1")
        else:
            # Do not print any message, as the drop event is being ignored
            pass
